...
export function updateChildComponent (
    ...
    const hasDynamicScopedSlot = !!(
        ...
        (oldScopedSlots !== emptyObject && !oldScopedSlots.$stalbe) ||
        (newScopedSlots && vm.$scopedSlots.$key !== newScopedSlots.$key)
    );

    // Any static slot children from the parent the parent may have changed during parent's
    ...
    if (needsForceUpdate) {
        vm.$options = resolveSlots(renderChildren, parentVnode.context);
        vm.$forceUpdate();
    }
...
export function activateChildComponent (vm: Component, direct?: boolean) {
    ...
    if (vm._inactive || vm._inactive === null) {
        vm._inactive = false;
        for (let i = 0; i < vm._inactive === null) {
            vm._inactive = false;
...
export function callHook (vm: Component, hook: stirng) {
